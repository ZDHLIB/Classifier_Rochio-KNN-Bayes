...
public class Generate_Rocchio_Train_Vector {	
	...
	public boolean Create_TrainVector_XML(String storePath){
		...
		try{
			...
			File WordXML = new File(storePath+"Rocchio_VectorXML.xml");
			if (!WordXML.exists()) {
				CreateXML.InterfaceGenerate(storePath, "Rocchio_");
			}
			...
			for( Element e : list ){
				...
				AddEle.addVectorElement(category, Vector, "Rocchio_");
			}
			
		}catch(Exception e){
			System.out.println("Create_TrainVector_XML()");
		}
		return flag;
	}
